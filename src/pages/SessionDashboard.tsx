...
import { PaidLeaguePanel } from '../components/PaidLeaguePanel';
...
export function SessionDashboard({ sessionPayload, user, mySessions, matchResults, standings, rounds, sessionUrl, onUpdateMatch, onDeleteSession, onBackToHome }: Props) {
  ...
  return (
    <motion.div key="dashboard" className="session-page" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
      ...
      <div className="dashboard-grid"><div className="dashboard-main"><StandingsTable standings={standings} format={sessionPayload.format} /><FixturesList rounds={rounds} format={sessionPayload.format} matchResults={matchResults} onUpdate={onUpdateMatch} /></div><aside className="dashboard-side"><SharePanel sessionUrl={sessionUrl} />{sessionPayload.paid?.enabled && <PaidLeaguePanel leagueId={sessionPayload.id} paid={sessionPayload.paid} standings={standings} user={user} isHost={Boolean(user && mySessions.some((s) => s.id === sessionPayload.id))} tournamentComplete={completed === fixtures.length} />}<div className="room-card"><div className="room-card-heading"><Share2 size={16} /> Host tools</div><button className="btn btn-ghost" onClick={() => navigator.clipboard?.writeText(sessionUrl)}><Copy size={15} /> Copy invite link</button><button className="btn btn-ghost" onClick={() => setTvMode(true)}><Monitor size={15} /> Open TV display</button></div></aside></div>
    </motion.div>
  );
}
